...
class PlateRecognizer:
    ...
    # (Keep validate_indian_plate as is, it's good for debugging now)
    # The validate_indian_plate function is already set up to show what regex it's checking.
    # We need to make sure the cleaned text actually becomes a valid format first.
    ...
    def recognize_plate(self, img):
        ...
        try:
            ...
            for r in results.boxes.data.tolist():
                ...
                # If using a general YOLO model, filter by class ID
                # if int(class_id) == license_plate_class_id and score > 0.5: # Adjust score threshold
                #     if score > best_conf:
                #         best_conf = score
            ...
            if best_bbox:
                ...
                # YOLO results.boxes.xywh for example gives center x,y, width, height.
                # If your YOLO model provides oriented bounding boxes or precise corners (not just rect),
                # you can use those for `four_point_transform`.
                # For standard YOLO rectangular boxes, a simple crop is often sufficient.
                # If the image quality is poor or angle is steep, you might need to
                # re-detect contours within the plate_roi for precise 4 points,
                # or find a YOLO model that outputs rotated bounding boxes.
                
                # For this example, we'll assume YOLO gives a good enough crop,
                # or you've extracted 4 precise corners for 'pts' below.
                # If YOLO just gives rectangular boxes, plate_roi is already cropped.
                
                # If you need perspective correction for non-rectangular detections:
                # You'd need to convert YOLO's detection to 4 corner points (tl, tr, br, bl).
                # This is a bit more advanced than simple YOLOv8 box output.
                # For now, let's assume `plate_roi` is good enough from straight crop.
                
                # You might need to implement a small contour detection *within* plate_roi
                # to get the exact 4 corners for `self.four_point_transform` if the
                # plate is heavily skewed inside the YOLO bbox.
                
                # For a simple rectangular crop from YOLO, you don't typically do
                # four_point_transform unless you identify the exact corners.
                # Let's add a placeholder for it, assuming you could get precise points
                # if your YOLO detection provided them, or you do a sub-detection.

                # If you need proper perspective correction, you'd extend this part:
                # Example: Assume `plate_roi_gray = cv2.cvtColor(plate_roi, cv2.COLOR_BGR2GRAY)`
                # Then run Canny/findContours on `plate_roi_gray` to get the actual plate contour
                # and its `approx` (4 corners).
                
                # For now, we'll just use the cropped `plate_roi` directly for EasyOCR.
                # If YOLO provides precise corners (e.g., from rotated bounding box models),
                # you would use them with four_point_transform.

                # ----------------------------------------------------
                # Step 3: Recognize text using EasyOCR
                # EasyOCR expects BGR image or grayscale.
                results_ocr = self.easy_ocr_reader.readtext(plate_roi)

                raw_ocr_text_candidates = []
                for (bbox, text, prob) in results_ocr:
                    raw_ocr_text_candidates.append(text)
                    if self.DEBUG_MODE:
                        # Draw OCR bounding box on the plate_roi for detailed debug
                        p0, p1, p2, p3 = bbox
                        cv2.rectangle(plate_roi, (int(p0[0]), int(p0[1])), (int(p2[0]), int(p2[1])), (0, 255, 255), 1) # Yellow boxes
                        cv2.putText(plate_roi, text, (int(p0[0]), int(p0[1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
                
                # Combine all recognized text lines into one string, separated by space or nothing
                # You might need more sophisticated logic here, e.g., if multiple lines detected.
            ...
            if self.DEBUG_MODE:
                display_text_for_debug = detected_plate_text if detected_plate_text else (raw_combined_text if plate_roi is not None else "No Plate Detected")
            ...
            if detected_plate_text:
                print(f"DEBUG: Logging to sheet: '{detected_plate_text}'", flush=True)
                ...
                print(f"DEBUG: No valid plate detected for logging.", flush=True)

        except Exception as e:
            if self.DEBUG_MODE:
                print(f"[ERROR] Plate recognition failed: {str(e)}", flush=True)
            # Ensure the debug output still shows something on error
            if self.DEBUG_MODE:
